# Role Permission Conditions Specification

## Purpose

Make role-permission grant conditions settable through the API (not just
seed data), protect the seeded system roles from deletion, and remove the
broken `permissionIds` write path on `PATCH /roles/:id` in favor of the
dedicated `role-permissions` endpoints.
