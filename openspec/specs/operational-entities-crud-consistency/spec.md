# Operational Entities CRUD Consistency Specification

## Purpose

Ensure the operational-path modules — `assignments`, `attendance-history`, and
`time-tracking` (the `TimeLog` module) — follow the same response contract as the
`employees`/`branches` reference modules: query Prisma through an `include`/`select`
const in a `types.ts`, and return responses through a `mapper.ts`, rather than
handing raw Prisma results back from the service. Existing CRUD and custom-action
behavior (check-in/check-out, verify) must be preserved byte-for-byte.
