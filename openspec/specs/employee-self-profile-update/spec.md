# Employee Self Profile Update Specification

## Purpose

Let an authenticated User with a linked Employee record update a safe, narrow subset of their
own contact info (`phoneNumber`, `email`, `address`) without holding the general
`update:employees` permission and without any risk of touching role, branch assignment, hourly
rate, status, or another caller's record.
