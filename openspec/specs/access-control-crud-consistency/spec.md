# Access-Control CRUD Consistency Specification

## Purpose

Ensure the access-control modules — `permissions`, `roles`, and
`role-permissions` — follow the same mapper/types file conventions as the rest of
`src/modules/`: singular `<name>.mapper.ts` filenames, and a dedicated
`mapper.ts` + `types.ts` split (no duplicated inline Prisma-to-DTO mapping).
Behavior is unchanged — this is a naming and dedup cleanup only.
...
- **WHEN** `POST /role-permissions` (assign) and `GET /role-permissions/role/:roleId` (list) are both called
