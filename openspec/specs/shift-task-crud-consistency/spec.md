# Shift/Task CRUD Consistency Specification

## Purpose

Ensure the shift/task template-domain modules — `master-shift-templates`,
`sub-shift-templates`, `task-templates`, `master-shifts`, `sub-shifts`, and
`tasks` — follow the same response contract as the `employees`/`branches`
reference modules: query Prisma through an `include`/`select` const in a
`types.ts` (`satisfies Prisma.XInclude`), and return responses through a
`mapper.ts` (`XMapper.toDto`), rather than handing raw Prisma results back from
the service. Existing CRUD and custom-action behavior (master-shift generation,
task completion) must be preserved.
