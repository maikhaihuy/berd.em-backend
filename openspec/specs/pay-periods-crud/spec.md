# Pay Periods CRUD Specification

## Purpose

Provide CRUD and a guarded `OPEN → CLOSED → FINALIZED` lifecycle for `PayPeriod`,
the open/closed/finalized window that payroll entries are generated into. Follows
the `employees` reference module structure and the global `PermissionsGuard`.
