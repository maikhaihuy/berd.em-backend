...
- **Auto-provisioning a User on Employee creation**: `POST /employees` (`EmployeesService.create`) creates the `Employee` and a linked `User` atomically in one `$transaction` — `Employee.userId` is set at creation time, not backfilled later. If no `User` exists with the Employee's phone number, one is created with that phone number, the Employee's full name, a random one-time password (`PasswordService.generateOneTimeCredential()` — an 8-character code drawn from a 32-symbol unambiguous alphabet via `crypto.randomBytes`, not derived from the phone number or any other submitted field), the seeded `Employee` role (resolved by `name`, not a hardcoded id), `mustChangePassword: true`, and `mustChangePasswordExpiresAt` set `INITIAL_PASSWORD_TTL_DAYS` days out. The plaintext credential is returned exactly once, as `temporaryPassword` on the `POST /employees` response — never persisted in plaintext and never returned again — for the calling Admin/Manager to relay to the employee out of band. If a `User` with that phone number already exists, Employee creation is rejected with a `FieldValidationException` on `phoneNumber` — an Admin must resolve the collision manually; the existing `User` is never silently reused. Existing Employees created before this feature (`userId: null`) are not backfilled.
- **Forced password change**: a `User` with `mustChangePassword: true` (populated fresh per-request onto `AuthenticatedUserDto` in `JwtAccessStrategy.validate()`) is restricted by the global `ForcePasswordChangeGuard` (`src/modules/auth/guards/force-password-change.guard.ts`, registered in `authz.module.ts` between `JwtAccessGuard` and `PermissionsGuard`) to routes marked `@Public()` or `@AllowWhilePasswordChangeRequired()` — everything else 403s. In practice only `POST /auth/change-password` and `POST /auth/logout-all` carry that decorator, since `login`/`refresh`/`logout`/`logout-device` are already `@Public()` and never see this guard. `POST /auth/change-password` (`{ currentPassword, newPassword }`, authenticated, `@SkipPermissions()`) always validates `currentPassword` against the stored hash — even while flagged — before hashing and persisting `newPassword` and clearing both `mustChangePassword` and `mustChangePasswordExpiresAt`. The 403's machine-readable `PASSWORD_CHANGE_REQUIRED` code travels in the response body's `details.code`, not a top-level `code` field — `GlobalExceptionFilter` only forwards `message`/`source`/`details`/`errors` from an exception body, so a bare `code` key would be silently dropped. A flagged `User`'s one-time credential is time-bounded: `LocalStrategy.validate()` rejects `POST /auth/login` with a distinct `INITIAL_PASSWORD_EXPIRED` `details.code` (not the generic invalid-credentials error) once `mustChangePasswordExpiresAt` has passed, even if the submitted password is otherwise correct — an Admin must then call `POST /users/:id/reissue-initial-password` (`@RequirePermissions({ action: 'update', subject: 'users' })`) to generate and return a fresh one-time credential, which overwrites the old one and resets the expiry.
- **Password recovery**: `POST /auth/forgot-password` (`@Public()`, username only) always responds `200 OK` regardless of whether the username matches a `User`, so it never reveals account existence — if it matches, it issues a single-use `PasswordResetToken` (invalidating any prior unexpired ones for that user). Because the backend has no email/SMS delivery channel, the actual working recovery path is admin-assisted: `POST /users/:id/password-reset-token` (`@RequirePermissions({ action: 'update', subject: 'users' })`) lets an Admin generate a token and returns its raw value in the response, to relay to the User out of band. `POST /auth/reset-password` (`@Public()`, token + newPassword) completes either flow, validating the *specific* submitted token by `bcrypt.compare`-ing it against every unexpired stored hash (the same shape as `JwtRefreshStrategy.findValidToken`) rather than matching on expiry alone — the original pre-restoration implementation had that scoping bug. All three routes are `@Throttle`d like `/auth/login`. `PasswordResetTokenService` (`src/modules/auth/password-reset-token.service.ts`, its own small module) owns this issuance/consumption logic and is imported by both `AuthModule` and `UsersModule` — kept separate specifically so `UsersModule` doesn't have to import all of `AuthModule` (which already imports `UsersModule`, so that would be circular). This token-based flow is distinct from `POST /users/:id/reissue-initial-password` above — that one replaces an auto-provisioned account's still-unclaimed one-time password, not an already-claimed account's forgotten one.
...
Validated at boot by `src/common/env.validation.ts` (missing/invalid values throw on startup): `DATABASE_URL`, `JWT_ACCESS_SECRET`, `JWT_ACCESS_EXPIRATION`, `JWT_REFRESH_SECRET`, `JWT_REFRESH_EXPIRATION`, `CORS_ALLOWED_ORIGINS` are required; `AUTH_DEV_MODE`, `AUTH_DEV_SECRET` are optional but required together to use dev login, and rejected outright when `NODE_ENV=production`. `INITIAL_PASSWORD_TTL_DAYS` (optional, default `7`) controls how many days an auto-provisioned employee's one-time password stays valid before `POST /auth/login` starts rejecting it as expired — see Auth flows above. `.env.example` has placeholder values — `PORT` there is 3000 but the app defaults to 3001 if unset.
